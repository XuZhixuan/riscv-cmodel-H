...
#include "/opt/homebrew/include/iverilog/vpi_user.h"
#include "/opt/homebrew/include/iverilog/sv_vpi_user.h"
#include "bits.hpp"
#include "common.h"
#include <map>
#include <vector>
...
#define always(name, sig...)                                                                                                                           \
	extern "C" void __$trig_func_##name##__(void *, const vector<Bits> &sigs);                                                                         \
	...
	void __$trig_func_##name##__(void *, const vector<Bitss> &sigs)

extern char __init_func_ptr_section_start, __init_func_ptr_section_end;
#define initial(name)                                                                                                        \
	extern "C" void __$init_func_##name##__();                                                                               \
...
public:
	explicit SVObject(const char *s)
	{
		...
		if (!strcmp("testbench.test_u.ram_u.mem1", s))
		{
			INFO("testbench.test_u.ram_u.mem1 type: %d, size: %d\n", vpi_get(vpiType, vpi_h), vpi_get(vpiSize, vpi_h));
		}
		if (!strcmp("testbench.test_u.ram_u.mem", s))
		{
			INFO("testbench.test_u.ram_u.mem type: %d, size: %d\n", vpi_get(vpiType, vpi_h), vpi_get(vpiSize, vpi_h));
		}
	}
	SVObject()
	{
	...
	SVObject operator=(const Bits &op)
	{
		...
		time_s.high = 0;
		time_s.low = 0;
		vpi_put_value(vpi_h, &val, &time_s, vpiTransportDelay);
		return *this;
	}
	...
	// const Bitss operator~()
	// {
	// 	return Bitss(~this->to_bits());
	// }
	...
	SVObject operator()(size_t pos)
	{
		string s;
		ostringstream ostr;
		vpiHandle sub_array_h;

		// if (vpi_get(vpiType, vpi_h) == vpiRegArray || vpi_get(vpiType, vpi_h) == vpiNetArray)
		// {
		// 	sub_array_h = vpi_handle_by_index (vpi_h, pos);
		// }
