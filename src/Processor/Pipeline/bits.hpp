...
public:
	Bits(unsigned long int op, uint32_t size = 64)
	{
...
private:
	...
	void set_from_str(const char *str)
	{
		...
		if (!str || strlen(str) == 0)
			ERROR("Illegal initialization string\n");
	...
	void set_from_str(const char *str, int base, int size = -1)
	{
		const char *p_sv_b, *p_sv_d, *p_sv_h;
		...
		if (base == 2)
		{
			...
			else if (strlen(str) >= 4 && p_sv_b)
			{
				...
				if (size != -1 && size != this->size)
					ERROR("The size defined by the argument conflicts with the size defined in the string\n");
				mpz_set_str(mpz, p_sv_b + 2, 2);
			}
			else
			{
